// JS port of https://www.kevinbeason.com/smallpt/explicit.cpp
...
function radiance(spheres, r, depth, E = 1) {
    ...
    if (obj.m == DIFF) {
      ...
      for (let i = 0; i < spheres.length; i++) {
        ...
        let sw = vsub(s.p, x),
            su = cross(abs(sw.x) > .1 ? v3(0, 1, 0) : v3(1,0,0), sw).norm(),
            sv = cross(sw, su);
        let cos_a_max = sqrt(1 - s.r * s.r / vsub(x, s.p).dot(vsub(x,s.p)));
        let eps1 = rand(), eps2 = rand();
        let cos_a = 1 - eps1 + eps1 * cos_a_max;
        let sin_a = sqrt(1 - cos_a * cos_a);
        let phi = 2 * PI * eps2;
        let l = vadd(
            vadd(su.smul(cos(phi) * sin_a), sv.smul(sin(phi) * sin_a)),
            sw.smul(cos_a)
        ).norm();
        let [hit, _t, id] = intersect(spheres, Ray.of(x, l));
        if (hit && id == i) {  // shadow ray
          let omega = 2 * PI * (1 - cos_a_max);
          e = vadd(e, f.mult(s.e.smul(l.dot(nl) * omega)).smul(M_1_PI));  // 1/pi for brdf
        }
      }

      return vadd(
        vadd(obj.e.smul(E), e),
        f.mult(radiance(spheres, Ray.of(x, d), depth, 0)));
